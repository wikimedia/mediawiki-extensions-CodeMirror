...
var ParserException = class extends Error {
   ...
   *
   */
  constructor(message, from, to, warnings, severity) {
    super(message);
    this.from = from;
    this.to = to;
...
// analyzer/TokenType.ts
var TokenType = /* @__PURE__ */ ((TokenType2) => {
  TokenType2[TokenType2["EndOfStream"] = 0] = "EndOfStream";
  TokenType2[TokenType2["Identifier"] = 1] = "Identifier";
  TokenType2[TokenType2["Keyword"] = 2] = "Keyword";
  TokenType2[TokenType2["StringLiteral"] = 3] = "StringLiteral";
  TokenType2[TokenType2["IntLiteral"] = 4] = "IntLiteral";
  TokenType2[TokenType2["FloatLiteral"] = 5] = "FloatLiteral";
  TokenType2[TokenType2["Operator"] = 6] = "Operator";
  TokenType2[TokenType2["Parenthesis"] = 7] = "Parenthesis";
  TokenType2[TokenType2["SquareBracket"] = 8] = "SquareBracket";
  TokenType2[TokenType2["Comma"] = 9] = "Comma";
  TokenType2[TokenType2["StatementSeparator"] = 10] = "StatementSeparator";
  return TokenType2;
})(TokenType || {});
...
var _Token = class _Token {
   ...
   */
  constructor(type, value, start, len = value.length) {
    this.type = type;
    this.value = value;
    this.start = start;
    this.end = start + len;
    if (type === 0 /* EndOfStream */) {
      this.value = _Token.EOF;
    }
  }
  /**
...
// analyzer/Tokenizer.ts
var commentStartRegex = /(\s*)\/\*/uy;
var operatorsRegex = /[!=]={0,2}|[:<>]=?|\*{1,2}|[/+\-%&|^?]/uy;
var numberRegex = /0([xbo])([\dA-Fa-f]+)\b|(\d+(?:\.\d*)?|\.\d+)(?!\w)/uy;
var identifierRegex = /\w+/uy;
var whitespaces = /* @__PURE__ */ new Set([" ", "	", "\n", "\v", "\f", "\r"]);
var punctuationTokens = /* @__PURE__ */ new Map([
  ["(", 7 /* Parenthesis */],
  [")", 7 /* Parenthesis */],
  ["[", 8 /* SquareBracket */],
  ["]", 8 /* SquareBracket */],
...
var numberBases = /* @__PURE__ */ new Map([
  ["x", 16],
  ["b", 2],
  ["o", 8]
]);
var digitsInBases = /* @__PURE__ */ new Map([
  [2, /^[01]+$/u],
  [8, /^[0-7]+$/u],
  [16, /^[\da-f]+$/iu]
]);
var valueKeywords = /* @__PURE__ */ new Set(["true", "false", "null"]);
var conditionKeywords = /* @__PURE__ */ new Set(["if", "then", "else", "end"]);
var Tokenizer = class {
  /**
   * @param input The input string to tokenize.
   */
  constructor(input, relationKeywords = []) {
    this.input = input;
  ...
  getNextToken(startOffset) {
    ...
    if (operatorMatch) {
      return new Token(6 /* Operator */, operatorMatch[0], offset);
    }
    ...
    if (numberMatch) {
      ...
      if (number.includes(".")) {
        ...
        if (!Number.isNaN(numberValue)) {
          return new Token(5 /* FloatLiteral */, String(numberValue), offset, tokenLength);
        }
      } else if (!base || digitsInBases.get(base).test(number)) {
        const numberValue = parseInt(number, base);
        if (!Number.isNaN(numberValue)) {
          return new Token(4 /* IntLiteral */, String(numberValue), offset, tokenLength);
        }
      }
    }
    ...
    if (identifierMatch) {
      ...
      const tokenType = isKeyword ? 2 /* Keyword */ : 1 /* Identifier */;
      if (!isKeyword) {
  ...
  readStringLiteral(startOffset) {
    ...
    while (offset < l) {
      ...
      if (char === quoteChar) {
        return new Token(3 /* StringLiteral */, stringContent, startOffset, offset - startOffset + 1);
      } else if (char === "\\") {
        if (offset + 1 >= l) {
          break;
        } else {
...
var Parser = class {
  ...
  isKeyword(identifier) {
    var _a;
    return ((_a = this.dialect.keywords) == null ? void 0 : _a.includes(identifier)) || valueKeywords.has(identifier) || conditionKeywords.has(identifier);
  }
  /* Levels */
  /**
  ...
  doLevelSemicolon() {
    ...
    do {
      this.move();
      if (this.is(0 /* EndOfStream */) || this.is(7 /* Parenthesis */, ")")) {
        break;
      }
      if (this.is(10 /* StatementSeparator */)) {
        continue;
      }
      statement = this.doLevelSet();
    } while (this.is(10 /* StatementSeparator */));
    return statement;
  }
  /** Handles variable assignment. */
  doLevelSet() {
    if (this.is(1 /* Identifier */)) {
      const initialState = this.getState();
      const { current } = this;
      this.move();
      if (this.is(6 /* Operator */, ":=")) {
  ...
  doLevelConditions() {
    if (this.is(2 /* Keyword */, "if")) {
      this.move();
      this.doLevelBoolOps();
      if (!this.is(2 /* Keyword */, "then")) {
        this.throwExpectedNotFound("then");
      }
      this.move();
      this.doLevelConditions();
      if (this.is(2 /* Keyword */, "else")) {
        this.move();
        this.doLevelConditions();
      }
      if (!this.is(2 /* Keyword */, "end")) {
   ...
   */
  doLevelBinaryOps(ops, method) {
    let leftOperand = this[method]();
    while (this.is(6 /* Operator */, ops)) {
      leftOperand = null;
      this.move();
      this[method]();
    }
  ...
  doLevelCompares() {
    ...
    while (this.is(6 /* Operator */, allowedOps)) {
      leftOperand = null;
  ...
  doLevelKeywordOperators() {
    ...
    if (this.is(2 /* Keyword */, this.dialect.keywords)) {
      this.move();
      this.doLevelUnarys();
      return null;
    }
  ...
  doLevelUnarys() {
    if (this.is(6 /* Operator */, unaryOps)) {
      this.move();
  ...
  /** Handles parenthesis. */
  doLevelParenthesis() {
    if (this.is(7 /* Parenthesis */, "(")) {
      if (this.nextIs(7 /* Parenthesis */, ")")) {
        this.move();
        this.throwUnexpectedToken();
      }
  ...
  doLevelFunction() {
    var _a;
    if (this.is(1 /* Identifier */) && this.nextIs(7 /* Parenthesis */, "(")) {
      if (((_a = this.dialect.functions) == null ? void 0 : _a.includes(this.current.value)) === false) {
      ...
      if (this.nextIs(7 /* Parenthesis */, ")")) {
        ...
        do {
          ...
          if (setFlag) {
            setFlag = false;
            if ((thisArg == null ? void 0 : thisArg.type) === 3 /* StringLiteral */) {
              thisArg.type = 1 /* Identifier */;
              thisArg.value = thisArg.value.toLowerCase();
              this.throwInternal(thisArg);
              this.locals.add(thisArg.value);
            }
          }
        } while (this.is(9 /* Comma */));
        if (!this.is(7 /* Parenthesis */, ")")) {
          this.throwExpectedNotFound(")");
        }
      }
  ...
  doLevelAtom() {
    var _a, _b, _c, _d;
    const { current } = this, { type, value } = current;
    switch (type) {
      case 1 /* Identifier */:
        if ((_a = this.dialect.disabled) == null ? void 0 : _a.includes(value)) {
          this.throw("use of disabled", true);
        } else if ((_b = this.dialect.deprecated) == null ? void 0 : _b.includes(value)) {
          this.throw("use of deprecated", true, "warning");
        } else if (((_c = this.dialect.functions) == null ? void 0 : _c.includes(value)) || this.isKeyword(value)) {
          this.throw("incorrect use of internal", true);
        } else if (((_d = this.dialect.variables) == null ? void 0 : _d.includes(value)) === false) {
          this.throwUndefined();
        }
        break;
    ...
    this.move();
    return type === 3 /* StringLiteral */ ? current : null;
  }
  /**
   ...
   * @param severity
   */
  getException(token, message, severity, quiet = true) {
    const { type, value, start, end } = token;
    return new ParserException(
      `${message} ${TokenType[type]} ${value && JSON.stringify(value)}`,
      start,
      end,
      quiet ? void 0 : this.diagnostics,
      severity
    );
  }
  throw(message, quiet = false, severity) {
   ...
   */
  throwExpectedNotFound(expected) {
   ...
   */
  throwInternal(token) {
    const { functions, variables, deprecated, disabled } = this.dialect, { value } = token;
    if ((functions == null ? void 0 : functions.includes(value)) || (variables == null ? void 0 : variables.includes(value)) || (deprecated == null ? void 0 : deprecated.includes(value)) || (disabled == null ? void 0 : disabled.includes(value)) || this.isKeyword(value)) {
   ...
   * @param token The token being assigned to.
   */
  throwUndefined(token = this.current) {
    const { value } = token;
...
// analyzer/index.ts
var index_default = (input, dialect) => {
...
const isSpace = (code) => code === ch.Space || code >= 9 && code <= 13;
const isNum = (code) => code >= ch[0] && code <= ch[9];
const isAlphaNum = (code) => code === ch.Underscore
    || isNum(code)
    || code >= ch.A && code <= ch.Z
    || code >= ch.a && code <= ch.z;
const eat = (input, predicate) => {
    let word = '';
...
const startKeywords = new Set(['true', 'false', 'null', 'if']);
const updateData = (dialect) => {
    if (dialect) {
        // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
        dialect.keywords && (dialect.keywords = dialect.keywords.filter(k => !startKeywords.has(k) && !conditionKeywords.has(k)));
        Object.assign(data, dialect);
    }
};
const relations = new ExternalTokenizer(input => {
    const word = eat(input, isAlphaNum);
    if (data.keywords.includes(word)) {
        input.acceptToken(Rel);
    }
});
const tokens = new ExternalTokenizer(input => {
    ...
    parser: parser.configure({
        props: [
            ...
            ext_CodeMirror_lib.foldNodeProp.add({
                ...
                IfStatement(node) {
                    const then = node.getChild('then'), end = node.getChild('end');
...
const analyzer = ({ state: { doc } }) => {
    try {
        index_default(doc.toString(), data);
    }
    catch (e) {
        const { from: f, warnings = [] } = e;
        if (f === undefined) { // eslint-disable-line @typescript-eslint/no-unnecessary-condition
            throw e;
        }
        return [...warnings, e]
            .map(({ from, to = from, message, severity = 'error' }) => ({
            from,
            to,
            severity,
...
exports.abusefilter = abusefilter;
exports.abusefilterLanguage = abusefilterLanguage;
