...
LineDecoration.prototype.mapMode = exports.MapMode.TrackBefore;
...
class PointDecoration extends Decoration {
    constructor(spec, startSide, endSide, block, widget, isReplace) {
        ...
        this.mapMode = !block ? exports.MapMode.TrackDel : startSide <= 0 ? exports.MapMode.TrackBefore : exports.MapMode.TrackAfter;
    }
...
function byGroup(view, pos, start) {
    ...
    return (next) => {
        ...
        if (cat == exports.CharCategory.Space)
            cat = nextCat;
        return cat == nextCat;
    };
}
    ...
    let hoverState = StateField.define({
        ...
        update(value, tr) {
            ...
            if (value && tr.docChanged) {
                let newPos = tr.changes.mapPos(value.pos, -1, exports.MapMode.TrackDel);
                if (newPos == null)
                    return null;
...
GutterMarker.prototype.mapMode = exports.MapMode.TrackBefore;
...
function defaultPositionInfo(view, list, option, info, space, tooltip) {
    let rtl = view.textDirection == exports.Direction.RTL, left = rtl, narrow = false;
...
class FieldRange {
    ...
    map(changes) {
        let from = changes.mapPos(this.from, -1, exports.MapMode.TrackDel);
        let to = changes.mapPos(this.to, 1, exports.MapMode.TrackDel);
...
const closeBracketEffect = /*@__PURE__*/StateEffect.define({
    map(value, mapping) {
        let mapped = mapping.mapPos(value, -1, exports.MapMode.TrackAfter);
...
function handleSame(state, token, allowTriple, config) {
    ...
    let dont = null, changes = state.changeByRange(range => {
        ...
        else if (state.charCategorizer(pos)(next) != exports.CharCategory.Word) {
...
function canStartStringAt(state, pos, prefixes) {
    ...
    if (charCat(state.sliceDoc(pos - 1, pos)) != exports.CharCategory.Word)
        return pos;
    for (let prefix of prefixes) {
        let start = pos - prefix.length;
        if (state.sliceDoc(start, pos) == prefix && charCat(state.sliceDoc(start - 1, start)) != exports.CharCategory.Word)
            return start;
    }
    return -1;
}
...
function ltrAtCursor(view) {
    return view.textDirectionAt(view.state.selection.main.head) == exports.Direction.LTR;
}
/**
...
function moveBySubword(view, range, forward) {
    ...
    let cat = exports.CharCategory.Space, pos = range.from, steps = 0;
    ...
    let step = (next) => {
        ...
        if (nextCat == exports.CharCategory.Word && next.charCodeAt(0) < 128 && /[\W_]/.test(next))
            nextCat = -1; // Treat word punctuation specially
        if (cat == exports.CharCategory.Space)
            cat = nextCat;
        if (cat != nextCat)
            return false;
        if (cat == exports.CharCategory.Word) {
            if (next.toLowerCase() == next) {
                ...
                if (sawUpper && forward && categorize(ahead = view.state.sliceDoc(pos, pos + 1)) == exports.CharCategory.Word &&
    ...
    if (segmenter && cat == exports.CharCategory.Word && end.from == range.from + steps * (forward ? 1 : -1)) {
...
function insideWordBoundaries(check, state, from, to) {
    return (from == 0 || check(state.sliceDoc(from - 1, from)) != exports.CharCategory.Word) &&
        (to == state.doc.length || check(state.sliceDoc(to, to + 1)) != exports.CharCategory.Word);
}
// Whether the characters directly at the given positions are word characters
function insideWord(check, state, from, to) {
    return check(state.sliceDoc(from, from + 1)) == exports.CharCategory.Word
        && check(state.sliceDoc(to - 1, to)) == exports.CharCategory.Word;
}
...
function stringWordTest(doc, categorizer) {
    return (from, to, buf, bufPos) => {
        ...
        return (categorizer(charBefore(buf, from - bufPos)) != exports.CharCategory.Word ||
            categorizer(charAfter(buf, from - bufPos)) != exports.CharCategory.Word) &&
            (categorizer(charAfter(buf, to - bufPos)) != exports.CharCategory.Word ||
                categorizer(charBefore(buf, to - bufPos)) != exports.CharCategory.Word);
    };
}
...
function regexpWordTest(categorizer) {
    return (_from, _to, match) => !match[0].length ||
        (categorizer(charBefore(match.input, match.index)) != exports.CharCategory.Word ||
            categorizer(charAfter(match.input, match.index)) != exports.CharCategory.Word) &&
            (categorizer(charAfter(match.input, match.index + match[0].length)) != exports.CharCategory.Word ||
                categorizer(charBefore(match.input, match.index + match[0].length)) != exports.CharCategory.Word);
}
