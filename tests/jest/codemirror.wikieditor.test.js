...
describe( 'logEditFeature', () => {
	...
	it( 'should only log when preferences are not the same as the default', () => {
		...
		expect( spy ).toHaveBeenNthCalledWith( 2, 'prefs-activeLine' );
		expect( spy ).toHaveBeenNthCalledWith( 3, 'prefs-autofocus' );
		expect( spy ).toHaveBeenNthCalledWith( 4, 'prefs-bracketMatching' );
